...
To fit the model with a constant zero-inflation term:

<<glmmTMB2,cache=TRUE>>=
fm2_zip0 <- update(fm2_zip, ziformula = ~ 1)
@ 

<<ncoef,echo=FALSE>>=
nc <- length(fixef(fm2_zip0)$cond)
## ?? would like to include in \Sexpr{ } below but ... ???
@ 

This model has 6 coefficients for the
conditional (count) model, but only one for the zero-inflation probability.
Since there is only a single (intercept) parameter, we can inverse-link transform
it (\code{plogis}, since the link for the zero-inflation model is logit) to
get the zero-inflation probability:

<<glmmTMB_zi>>=
(zi_logit <- fixef(fm2_zip0)$zi)
(zi_prob <- plogis(zi_logit))
@ 
