...
are one approach to correcting for this issue.
...
You can use bootstrap or parametric bootstrap samples to get more
$p$-values/confidence intervals that account for : for example,
...
rownames(ptab) <- names(coef(model1))
print(ptab)
@
