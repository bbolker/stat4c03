...
\begin{equation*}
  \boxed{
\llik = (y \theta - b(\theta))/\phi + c(y,\phi)
}
\end{equation*}
...
\begin{itemize}
  \item 
    \emph{The \emph{score function}  $\frac{\partial \llik}{\partial \theta}$ has expected value zero.}
...
\item multidimensional Newton-Raphson estimation:
  iterate solution of $\bm A \Delta \bb = \bm u$ where $\bm A$ is
  the negative of the
...
& = \X^T \bm W (\X  \bb) + \X^T \bm W (y-\mu) \frac{d\eta}{d\mu} \\ 
